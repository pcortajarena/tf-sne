...
The second application consists on the computation of nearest neighbours in a low-dimensional space. This second implementation focuses on Multidimensional Space algorithm. The aim is to obtain the best aproximate nearest neighbours within a reduced map of datapoints compared to the original map, carrying out a reusable, quicker and low memory intensive algorithm.

\subsection{Dataset}
\label{ssec:data2}

The dataset \textit{"forests.csv"} is used for this application and is extracted from UCI Machine Learning Repository \citep{ucidata}. As this dataset turns to have too many instances, a random subsample of 20.000 examples is storaged with 55 features each. This are all numeric variables describing the type of forest the concrete example is. The forest type output is represented by a number in range 1 to 7, shaping the last column of the dataset.\\
The reason why this dataset is chosen for this concrete application relies on the need to compute a lot of examples in the training phase to obtain a reliable model and to be able to compare time and memory performance.

\subsection{Algorithm: Nearest Neighbours}
\label{ssec:nneigh}

The main task contemplated in this application is nearest neighbours obtaining. The following research bears with reusability and time and memory reducing algorithms.

Regarding the first point, because of MDS being non-parametric, it is uncapable of obtaining new reduced points without computing again all the operations inside. This means if we have already output the low-dimensional map of points from the dataset, it would not be posible to obtain a new point in this map unless the algorithm is recomputed. If we based our nearest neighbours search in a previous dimensionality reduction algorithm such as MDS, the only results available would be from the points just computed by MDS, forgetting about new example posibilities.

Nearest neighbours models are known to be eficiently poor when a raw dataset is provided, both because the memory this data occupies tends to be big and thus the computational time required also increases.
