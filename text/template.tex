...
\caption{\label{nnet}Dimensionality reduction algorithm learned by a Neural Network}
\end{figure}

\newpage

\section{Class: NNReplicator}
\label{sec:nnrep}

We have developed a class named NNReplicator which replicates the behaviour of a neural net wrapping an embedder object, as it is represented in figure \ref{nnet}. It is written as if it was a Scikit Learn class so that the objects created could fix into a Scikit Learn pipeline, being one of the main future applications.\\
The inputs expected in this new class are the next ones:
\begin{itemize}
\item n\_comp: number of components for the low-dimensional space
\item embedder: dimensionality reduction object, such as PCA, MDS or TSNE in our examples
\item layers: array with the number of neurons for each layer, taking into account that the last layer has the same neurons as the number of components in the low dimensional space; e.g: [100,10]: it has three layers, with 100, 10 and n\_comp neurons each.
\item dropout: array with the same size as layers, containing the dropouts for each layer.
\item lr: learning rate for the neural network training.
\item act\_function: activation function for the neural net.
\item loss\_func: losses function used to minimize the cost.
\item epochs: number of epochs for each training
\item batch\_size: number of examples used to compute the cost function.
\end{itemize}
Once the inputs are understood, the next point is to get to know what is being computed and what methods are included in the class. As we want to include it as a Scikit Learn class, some methods are required; those are the method fit and transform, which make the class capable of instantiate tranformers. This is explained in sections \ref{ssec:nnc}, \ref{ssec:fit} and \ref{ssec:trans}.

\subsection{Methods: nnConstruct}
\label{ssec:nnc}
...
\subsection{Methods: fit}
\label{ssec:fit}

\subsection{Methods: transform}
\label{ssec:trans}
