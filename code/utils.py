...
def calculate_corrcoef(y_true, y_pred):
    corrcoefs = [np.corrcoef(y_true[:, i], y_pred[:, i])[0, 1]
...
if __name__ == '__main__':
    ...
    md = MDS(n_components=n_comp, random_state=0,
             n_jobs=-1, verbose=10, n_init=4)
    ...
    knn_perc_Xtrans = knn_percentage_preserved(
        X_trans, cv_preds, n_true=10, n_pred=100)
